...
#' coercions we want to allow:
#'
#' * Numeric, integer, or logical to string
#' * Specific strings to logical: "TRUE", "True", "true", "FALSE", "False",
#' "false"
#'
...
can_coerce <- function(values, class) {
  if (class == "character" &
    ...
    return(TRUE)
  } else if (class == "logical" & inherits(values, "character")) {
    logicals <- c("TRUE", "True", "true", "FALSE", "False", "false")
    if (all(values %in% logicals)) {
      return(TRUE)
    } else {
      return(FALSE)
    }
